...
import { RecipeFormIError } from '../../types/recipe';
import { useDispatch, useSelector } from 'react-redux';
import { AppDispatch, RootState } from '../../types/redux';
import { regularValidation } from '../../controller/validation';
import { changeRecipeForm, reseteRecipeForm } from '../../redux/recipe';
import { StyleSheet, View, Text, ScrollView, Pressable, Image } from 'react-native';

interface PropsI {
  Title: string;
  navigation: NavigationI;
  submitFunction: (recipe:  FormData) => void;
}
...
export default function EditCreateForm({ navigation, Title, submitFunction }: PropsI) {
	const newRecipe: FormData = new FormData();
	const [key, setKey] = useState<string>('');
	const dispatch: AppDispatch = useDispatch();
	const [error, setError] = useState<RecipeFormIError>({
		ingredients: null,
		steps: null,
		title: null,
		description: null,
	...
	useEffect((): void => {
		Title === 'Edit'
			? setButtonStatus(
				Object.values(error).some((el: boolean) =>
					el === null ? false : el === false ? true : false
				)
			)
			: setButtonStatus(
				Object.values(error).every((el: boolean) =>
					el === null ? false : el === false ? true : false
				)
			);
	}, [error]);
	...
	const createNewRecipeFormData = (): void => {
		...
		if (!regularValidation(recipe.steps)) newRecipe.append('steps', JSON.stringify(recipe.steps));
		if (!regularValidation(recipe.image)) newRecipe.append('image', JSON.stringify(recipeBuffer));
		...
		if (!regularValidation(recipe.ingredients)) newRecipe.append('ingredients', JSON.stringify(recipe.ingredients));
	};
		...
		<ScrollView style={styles.container}>
			...
			<View style={styles.form}>
				<Pressable onPress={pickImage}>
					{recipe.image !== '' ? (
						<Image
							style={styles.addImgArea}
							source={{ uri: getImgUrl(recipe.image) }}
						/>
					) : (
				...
				<View style={styles.picker}>
					<Picker selectedValue={recipe.type} onValueChange={pickerFunc}>
